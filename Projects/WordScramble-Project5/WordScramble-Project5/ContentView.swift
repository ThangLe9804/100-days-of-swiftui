...
struct ContentView: View {
    ...
    func addNewWord() {
        ...
        guard isNotTooShort(word: answer) else {
            wordError(title: "That's too short", message: "Should be >= 3 chars")
            return
        }

        guard isNotGivenWord(word: answer) else {
            wordError(title: "Try another", message: "That's what we give you")
            return
        }
